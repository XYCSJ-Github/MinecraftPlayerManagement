...
public:
	...
	inline const WorldDirectoriesNameList GetWorldList(void) { if (this->uct_world_list.world_directory_list.size() == 0 && this->uct_world_list.world_name_list.size() == 0) { throw NullStruct(); return this->uct_world_list; } }//获取世界列表容器结构体
...
private:
	...
	[[deprecated("统一列表的使用，所以该函数被废弃")]]
	inline void SetWorldList(WorldDirectoriesNameList _uct_world_list) { if (_uct_world_list.world_directory_list.size() == 0 && _uct_world_list.world_name_list.size() == 0) { throw NullStruct(); } this->uct_world_list = _uct_world_list; }//设置世界列表容器结构体
